...
def para_set():
    ...
    if input('Do u set the parameter(y/n):') == 'y':
        ...
        if p_dic['method'] == '高斯自适应阈值二值化':
            try:p_dic['blur_num'] = int(input('均值滤波卷积核(1,3,5,7)当前为:%s:'%p_dic['blur_num']))
            except:None
            try:p_dic['guassian_wide'] = int(input('高斯二值化采样区(3-30):%s:'%p_dic['guassian_wide']))
            except:None
            try:p_dic['gaussian_decrement'] = int(input('高斯二值化阈值差量:%s:'%p_dic['gaussian_decrement']))
            except:None
        elif p_dic['method'] == 'canny':
            try:p_dic['canny_threshold1'] = int(input('canny低阈值(50-110):%s:'%p_dic['canny_threshold1']))
            except:None
            try:p_dic['canny_threshold2'] = int(input('canny高阈值(150-200)%s:'%p_dic['canny_threshold2']))
            except:None
            try:p_dic['canny_wide'] = int(input('canny卷积核:%s:'%p_dic['canny_wide']))
            except:None
        elif p_dic['method'] == 'laplacian' or p_dic['method'] == 'sobel':
            try:p_dic['soble_size'] = int(input('sobel卷积核:%s:'%p_dic['soble_size']))
            except:None
            if input('laplacian or sobel处理是否启用二值化(y/n):%s:'%p_dic['flag_binary'])=='y':
                p_dic['flag_binary'] = True
                try:p_dic['binary_threshold'] = int(input('二值化阈值:%s:'%p_dic['binary_threshold']))
                except:None
            else:p_dic['flag_binary'] = False
    print(p_dic)
    Pcer.set_config(p_dic)
...
def select():
    ...
    try:o_num = int(input('order number of picture u want(from 0 to n):'))
    except:o_num = 0
    return o_num

def process(img, method):
    
    if method == '高斯自适应阈值二值化':img = Pcer.method_binarization(img)
    elif method == 'sobel':img = Pcer.method_sobel(img)
    elif method == 'laplacian':img = Pcer.method_laplacian(img)
    elif method == 'canny':img = Pcer.method_canny(img)
